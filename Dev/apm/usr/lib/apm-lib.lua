...
function Package:new(dir)
  ...
  instance.dir=dir
  --manifest
  if not fs.exists(dir.."/manifest") then return nil end
  local manifest=io.open(dir.."/manifest")
  instance.name=manifest:read("l")
  if not instance.name then return nil end
  instance.ver=manifest:read("l")
  if not instance.ver then return nil end
  local line=manifest:read("l")
  instance.files={}
  if not line then return nil end --cannot have no packages
...
function apm.ver() return ver end
...
function apm.getPacket(server_ip,name,pretty)
  if not require("ipv2").isIPv2(server_ip) or not name then return false end
  ...
  local rdata=mnp.receive(server_ip,"apm",10)
  if not rdata then return false,"mnp-timeout" end
  if rdata[1]=="transmission-start" then
    local files=rdata[2]
    local size=rdata[3]
    --confirm size(maybe get-info?)
    if not ftp.serverConnectionAwait(server_ip,10) then
    ...
    for i=1,#files do
      local fdata=mnp.receive(server_ip,"ftp",10)
      if fdata[1]=="put" then
        local filename=fdata[3][1]
        if not filename then mnp.log("APM","Failed to fetch filename of packet: ",2)
        else
          if pretty then cprint(">>Getting "..files[i],0xFFFF33) end
...
function apm.server(packageDir)
  if not packageDir then packageDir="/" end
  ...
  while true do
    ...
    if id=="interrupted" then
      ...
      if rdata[1]=="get-ver" then
        local name=rdata[2]
        if packages[name] then
          mnp.send(from_ip,"apm",{packages[name].ver})
        else
